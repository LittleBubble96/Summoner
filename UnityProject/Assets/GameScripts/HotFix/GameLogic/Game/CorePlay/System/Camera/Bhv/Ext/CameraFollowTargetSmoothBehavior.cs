...
{
    public class CameraFollowTargetSmoothBehavior : CameraBehavior
    {
        ...
        public override void OnExecute(Camera camera, float dt)
        {
            ...
            transform.position = Vector3.Lerp(transform.position, desiredPosition, dt * 5f);
            Quaternion desiredRotation = _rotationOffset;
            transform.rotation = Quaternion.Slerp(transform.rotation, desiredRotation, dt * 5f);
        }
