...
namespace UnityGameFramework.Runtime
{
    ...
    public sealed class EventComponent : GameFrameworkComponent
    {
        ...
        public void Fire(object sender,int eventId)
        {
            GameEventCustom eventCustom = ReferencePool.Acquire<GameEventCustom>();
            eventCustom.CustomId = eventId;
            m_EventManager.Fire(sender, eventCustom);
        }
        ...
        public void Fire<T>(object sender,int eventId ,T param)
        {
            GameEventCustomOneParam<T> eventCustom = ReferencePool.Acquire<GameEventCustomOneParam<T>>();
            eventCustom.CustomId = eventId;
        ...
        /// 抛出事件立即模式，这个操作不是线程安全的，事件会立刻分发。
        /// </summary>
        public void FireNow<T>(object sender, int eventId ,T param)
        {
            GameEventCustomOneParam<T> eventCustom = ReferencePool.Acquire<GameEventCustomOneParam<T>>();
            eventCustom.CustomId = eventId;
            eventCustom.Param = param;
